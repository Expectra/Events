...
def inscrever_evento(request, id):
    ...
    if request.method == "GET":
        ...
        if request.user == evento.participantes:
            messages.add_message(request, constants.INFO, 'Você já está participando desse evento.')
            return render(reverse('/inscrever_evento'))
...
def remover_evento(request, id):
    if not evento.criador == request.user:
        ...
        return redirect(reverse('/gerenciar_evento/'))
    
    evento = get_object_or_404(Evento, id=id)
    evento.delete()
    messages.add_message(request, constants.SUCCESS, 'Evento removido com sucesso!')
    return redirect(reverse('/gerenciar_evento/'))
